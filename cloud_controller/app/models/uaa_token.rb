  ...
  class << self
    ...
    def decode_token(auth_token)
      ...
      begin
        ...
        CloudController.logger.debug("token key fetched from the uaa has algorithm #{@token_key.inspect}") if @token_key[:alg] == "SHA256withRSA"
        public_key = @token_key[:value]
        @uaa_token_coder = CF::UAA::TokenCoder.new(AppConfig[:uaa][:resource_id],
                                                   AppConfig[:uaa][:token_secret],
                                                   public_key)
      rescue => e
        @token_key_fetch_failure_count -= 1
        CloudController.logger.error("Failed to fetch the token key from the UAA token_key endpoint or invalid key fetched")
        CloudController.logger.debug("Request to uaa/token_key OR coder init failed. #{@token_key_fetch_failure_count} retries remain. #{e.message}")
      end unless @token_key_fetch_failure_count == 0
