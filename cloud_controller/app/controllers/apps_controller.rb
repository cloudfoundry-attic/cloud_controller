    ...
    if body_params && body_params[:staging]
      ...
      if body_params[:staging][:model]
        ...
        if runtime=body_params[:staging][:runtime]
          #TODO handle possibility this is a String identifier.  The method will return nil if no runtime found
          app.runtime = StagingPlugin.runtime_id runtime[:name],runtime[:version]
          #TODO unless app.runtime - complain, the name and version were not valid and app will just set to framework default instead
        end
      end
